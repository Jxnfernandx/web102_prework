...
function addGamesToPage(games) {
    ...
    // loop over each item in the data
        games.forEach(({name, backers, img, description}) => {
            gamesContainer.innerHTML += `
            <div class="game-card">
                <img class="game-img" src=${img}>
                <div class="name">${name}</div>
                <span class="description">${description}</span>
                <div class="backers">Backers: ${backers}</div>
            </div>
            `;
        });
...
const contributers = GAMES_JSON.reduce((acc, games) => {
...
function filterUnfundedOnly() {
    ...
    let fundingNotYetReached = GAMES_JSON.filter((game) => {
        return game.pledged < game.goal;
    });

    // use the function we previously created to add the unfunded games to the DOM
    addGamesToPage(fundingNotYetReached)
}
...
const displayStr = countUnfunded = 1
? `<p> A total of $${amountRaised.toLocaleString('en-US')} has been raised for ${numOfGames} games. Currently, ${countUnfunded} remain unfunded.
We need your help to fund these amazing games! </p>`
: `<p> A total of $${amountRaised.toLocaleString('en-US')} has been raised for ${numOfGames} games. Currently, ${countUnfunded} remains unfunded.
We need your help to fund these amazing games! </p>`
